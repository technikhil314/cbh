...
describe("deterministicPartitionKey", () => {
  it("Returns the literal '0' when given no input or false liks input", () => {
  ...
  it("return sha if partition key is false like", () => {
  ...
  it("return same partition key if length is less than or equals MAX_PARTITION_KEY_LENGTH", () => {
    let hash = deterministicPartitionKey({
      partitionKey: "demo"
    });
  ...
  it("return hash if partition key length is greater than MAX_PARTITION_KEY_LENGTH", () => {
  ...
  it("return hash if partition key is not string", () => {
    const hash = deterministicPartitionKey({
      partitionKey: {
  ...
  it("return hash if partition key is not string but sealized json length is less that MAX_PARTITION_KEY_LENGTH", () => {
    const partitionKey = {
      demo: "demo"
    }
    const hash = deterministicPartitionKey({
